package com.biology.common.utils.poi;
...
public class CustomExcelUtil {
    ...
    public static <T> void writeToResponseForImportTemp(List<T> list, Class<T> clazz, HttpServletResponse response) {
        try {

            writeToOutputStreamForImportTemp(list, clazz, response.getOutputStream());
        } catch (IOException e) {
            throw new ApiException(e, Internal.EXCEL_PROCESS_ERROR, e.getMessage());
        }
    }
    ...
    public static <T> List<T> readFromResponseForImportTemp(Class<T> clazz, MultipartFile file) {
        try {
            return readFromResponseForImportTemp(clazz, file.getInputStream());
        } catch (IOException e) {
            throw new ApiException(e, Internal.EXCEL_PROCESS_ERROR, e.getMessage());
        }
    }

    public static <T> void writeToOutputStreamForImportTemp(List<T> list, Class<T> clazz, OutputStream outputStream) {
        ...
        for (Field field : fields) {
            ...
            if (annotation != null && annotation.showInImportTemplate()) {
    ...
    public static <T> List<T> readFromResponseForImportTemp(Class<T> clazz, InputStream inputStream) {
        ExcelReader reader = ExcelUtil.getReader(inputStream);
        // 去除掉excel中的html标签语言 避免xss攻击
        reader.setCellEditor(new TrimXssEditor());
