package com.biology.admin.customize.service.login;
...
import com.biology.common.constant.Constants.Captcha;
...
import com.biology.common.utils.i18n.MessageUtils;
import com.biology.domain.common.cache.GuavaCacheService;
import com.biology.domain.common.cache.MapCache;
import com.biology.domain.common.cache.RedisCacheService;
import com.biology.admin.customize.async.AsyncTaskFactory;
import com.biology.infrastructure.thread.ThreadPoolManager;
...
import com.biology.common.enums.common.LoginStatusEnum;
...
public class LoginService {
    ...
    public String login(LoginCommand loginCommand) {
        // 验证码开关
        if (isCaptchaOn()) {
            validateCaptcha(loginCommand.getUsername(), loginCommand.getCaptchaCode(),
                    loginCommand.getCaptchaCodeKey());
        }
        // 用户验证
        ...
        try {
            ...
            ThreadPoolManager
                    .execute(AsyncTaskFactory.loginInfoTask(loginCommand.getUsername(), LoginStatusEnum.LOGIN_FAIL,
            ...
            ThreadPoolManager.execute(AsyncTaskFactory.loginInfoTask(loginCommand.getUsername(),
                    LoginStatusEnum.LOGIN_FAIL, e.getMessage()));
    ...
    public CaptchaDTO generateCaptchaImg() {
        ...
        if (isCaptchaOn) {
            ...
            // 生成验证码
            String captchaType = BiologyConfig.getCaptchaType();
    ...
    public String decryptPassword(String originalPassword) {
        byte[] decryptBytes = SecureUtil.rsa(BiologyConfig.getRsaPrivateKey(), null)
