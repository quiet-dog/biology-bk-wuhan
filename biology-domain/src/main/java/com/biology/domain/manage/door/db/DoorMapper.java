package com.biology.domain.manage.door.db;

import org.apache.ibatis.annotations.Select;

import com.baomidou.mybatisplus.core.mapper.BaseMapper;

public interface DoorMapper extends BaseMapper<DoorEntity> {
        // 获取今天进入的人数

        @Select("SELECT COUNT(personnel_id) FROM manage_door WHERE DATE_FORMAT(create_time, '%Y-%m-%d') = CURDATE()"
                        + " GROUP BY personnel_id")
        public Integer getDayStatus();

        /**
         * SELECT COUNT(*)
         * FROM (
         * SELECT 1
         * FROM manage_door
         * WHERE door_date >= UNIX_TIMESTAMP(CURDATE())
         * AND door_date < UNIX_TIMESTAMP(CURDATE() + INTERVAL 1 DAY)
         * AND enter_status = '通行'
         * GROUP BY name, department
         * ) AS t;
         * 
         * @return
         */
        // @Select("SELECT COUNT(*) AS count FROM manage_door WHERE door_date >=
        // UNIX_TIMESTAMP(CURDATE())"
        // + " AND door_date < UNIX_TIMESTAMP(CURDATE() + INTERVAL 1 DAY)"
        // + " AND enter_status = #{enterStatus} Group by par")
        @Select("SELECT COUNT(*) as count FROM (SELECT 1 FROM manage_door WHERE "
                        + " DATE(create_time) = CURDATE()"
                        + " AND enter_status = #{enterStatus} Group by name, department) AS t")
        public Integer getChuQinLvDay(String enterStatus);
}
