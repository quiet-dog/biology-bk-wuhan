...
public interface EventMapper extends BaseMapper<EventEntity> {
        ...
        // 获取区域统计
        @Select("SELECT COALESCE(q.installation_location, n.e_area) AS manufacturer,COUNT(*) as count FROM manage_event as e "
                        + " LEFT JOIN manage_equipment q on q.equipment_id = e.equipment_id AND q.deleted = 0 "
                        + " LEFT JOIN manage_environment n on n.environment_id = e.environment_id AND n.deleted = 0"
                        // + " WHERE e.create_time >= CURDATE() - INTERVAL 6 DAY AND e.create_time <
                        // CURDATE() + INTERVAL 1 DAY"
                        + " WHERE e.create_time >= DATE_ADD(CURDATE(), INTERVAL 1 - DAYOFWEEK(CURDATE()) DAY)"
                        + " AND e.create_time < DATE_ADD(CURDATE(), INTERVAL 8 - DAYOFWEEK(CURDATE()) DAY)"
