...
public interface CaiYangDataMapper extends BaseMapper<CaiYangDataEntity> {

        @Select("SELECT DATE_FORMAT(m.minute, '%Y-%m-%d %H:%i') AS minute," +
                        "CASE WHEN COUNT(d.device_sn) > 0 THEN 1 ELSE 0 END AS is_online " +
                        "FROM ( " +
                        ...
                        ") m " +
                        "LEFT JOIN manage_cai_yang_data d ON d.device_sn = #{deviceSn} " +
